function stf_dao = matRad_generateStf(ct,cst,pln,mlc,visMode)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% matRad steering information generation
% 
% call
%   stf_dao = matRad_generateStf(ct,cst,pln,mlc,visMode)
%
% input
%   ct:         ct cube
%   cst:        matRad cst struct
%   pln:        matRad plan meta information struct
%   mlc:        the mlc physical contraints
...
fprintf('matRad: Generating stf_dao struct... ');
...
addmarginBool = 1;
if addmarginBool
    voiTarget = matRad_addMargin(voiTarget,cst,ct.resolution,mlc.targetMargin,true);
    V   = find(voiTarget>0);
end
...
for i = 1:length(pln.gantryAngles)
    ...
    stf(i).fitShape = getFitShapeStructure( coordsAtIsoCenterPlane(:,1) ,coordsAtIsoCenterPlane(:,2), mlc);

    [rayPos,xWidth,yWidth] = getRayPositionVectorFromFitshape( stf(i).fitShape,mlc);
    ...
    for j = 1:stf(i).numOfRays
        ...
        if strcmp(pln.radiationMode,'photons') 
            stf(i).ray(j).rayCorners_SCD = (repmat([0, machine.meta.SCD - SAD, 0],4,1)+ (machine.meta.SCD/SAD) * ...
                                                             [rayPos(j,:) + [+stf(i).bixelWidth/2,0,+ yWidth(j)/2.0];...
                                                              rayPos(j,:) + [-stf(i).bixelWidth/2,0,+ yWidth(j)/2.0];...
                                                              rayPos(j,:) + [-stf(i).bixelWidth/2,0,- yWidth(j)/2.0];...
                                                              rayPos(j,:) + [+stf(i).bixelWidth/2,0,- yWidth(j)/2.0]])*rotMx_XY_T*rotMx_XZ_T;
        end
    end
